...
onMounted(async () => {
  ...
  // 1. 创建空心圆柱（杯子主体）
  const cupBody = BABYLON.MeshBuilder.CreateCylinder(
    'cupBody',
    {
      height: 2,
      diameterTop: 1,
  ...
  // 2. 创建一个圆盘作为杯底
  const cupBottom = BABYLON.MeshBuilder.CreateDisc(
    'cupBottom',
    {
  ...
  // 3. 调整杯底位置（放在圆柱底部）
  cupBottom.position.y = -10 // 圆柱高度的一半（height/2）
